/**
 * war2 dev — deviates from the shared per-package dev server to also run war2's
 * PeerJS broker (:9000) and debug WebSocket server (:9229) in the same process
 * tree, replacing the old `concurrently` trio. Children run in this directory so
 * their deps (peer, ws) resolve here.
 */
import { spawn } from "node:child_process";
import { serve } from "../../../util/vite/dev";
