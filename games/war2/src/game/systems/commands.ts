...
/**
 * Apply a batch of commands to the sim world.
 * Commands carry stable unitIds (not bitecs eids) so they survive a full resync.
 */
export function applyCommands(world: SimWorld, cmds: Command[]): void {
    for (const cmd of cmds) {
        if (cmd.type === CmdType.MOVE) {
            ...
            spawnUnit(world, cmd.xFP, cmd.yFP, cmd.team, cmd.unitId, cmd.typeId);
            ...
            // Validate placement at apply-time on both sims (deterministic).
            if (canPlaceBuilding(world, cmd.tileX, cmd.tileY, cmd.typeId)) {
                spawnBuilding(world, cmd.tileX, cmd.tileY, cmd.team, cmd.typeId, cmd.unitId);
            }
        }
    }
}
