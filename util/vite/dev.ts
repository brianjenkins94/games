 ...
 *
 * A package opts in via its own `dev` script (`tsx ../../dev.ts`); this serves
 * *that* package (its cwd) with Vite in middleware mode (lib's getViteDevServer).
 * A package that needs more deviates with its own dev script that imports `serve`
 * and adds its services — see `games/war2/dev.ts` (PeerJS broker + debug server).
 */
...
import { getViteDevServer } from "@brianjenkins94/util/router";

/** Serve one game directory in dev (Vite middleware + manual HTML, since
